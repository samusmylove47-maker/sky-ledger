...
3. **~~`model4.py` line 67, `up10()`, applies the +1/tier floor to weapon damage.~~ RESOLVED 31 Aug 2026 — and the resolution is worse for us than the flag was.** The floor is gone from `model4.py` and `model3.py`; both now use `base + floor(base × tier / 10)`, matching `jt()` and Session B's `upgrade.ts`. Three things this flag got right and one it did not. Right: the floor was unmeasured, `jt` is right, and `OH_RATE_CAP = 1.42` *was* patching our own upgrade rule — `verify_upgrade.py` §5 runs the rankings under both rules with the cap on and off and only floor-with-cap-off differs, so under the percentage rule the cap is now inert and removing it changes nothing. Wrong: this flag said "one client window of any sub-10-damage weapon settles it" as though only *we* lacked the capture. Session B's five captures do not reach base 10 either. **Ten captures across two repositories, zero decisive**, and `EQUIPMENT-TRUTH.md` §2 claimed the floor was confirmed by a tooltip whose own text says the deciding row *ties*. The item to send outward is not "we fixed our upgrade rule" but "the floor term is ungraded on both sides and here is the one capture that grades it".
