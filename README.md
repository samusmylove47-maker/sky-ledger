...
**This repository holds two things.** It began as the Sky Ledger overlay app and is
now also the working tree of the EverQuest Legends measurement project — the trio
model, the gap engine, and the sourcing standard the rest of the project builds to.
Both are documented below. Read this section first, because the app's source files
are **not in this tree** and the instructions further down assume they are.

## 1 · The measurement work — what actually runs here
...
| `gapengine.py` | the gap engine — given a log, what stands between a trio and its own ceiling. Pure; `gap_engine(lines, context) -> Report`. |
| `bundle/` | the shipped JS build of that engine, its contract, and the checks that keep the two in step. |
| `derived/`, `derived_check.py` | the claim gate. A finding ships only if it is uncomputable from a catalogue. |
| `fixtures/` | the sample report, **generated by running the engine**, so its shape cannot drift from what a consumer renders. |
| `handover/` | notes to the other sessions in the project. Proposals across a boundary; nothing is ever pushed across one. |

**Reproducing a weapon number.** Three catalogue shards (`sh-PRIMARY.json`,
`sh-SECONDARY.json`, `sh-RANGE.json`) are fetched rather than committed.
`python3 fetch_shards.py` gets them and verifies their sha256 against the exact
bytes every published figure was computed from. `check.sh` runs it first.

**The check suite is a guard, not a gate, and says so in its own header.** It
becomes a gate in CI (`.github/workflows/check.yml`) and inside `gapEngine()`,
where a claim that fails the validator cannot reach a caller.

## 2 · The overlay app — documented here, source not in this tree

Everything from *"Fastest start"* down documents the Sky Ledger overlay: a Plane of
Sky class-unlock tracker for EverQuest Legends that sits on top of the game. Track
the tests you care about, see what each missing piece drops from, and get told which
quests you can hand in right now — without alt-tabbing.

> **`SkyLedger.html`, `package.json`, `main.js` and `preload.js` are not in this
> repository**, and neither are the client files `eqstr_us.txt` and `dbstr_us.txt`
> (those live in an EverQuest Legends install, not in any repo). **So the commands
> below will not run against a fresh clone of this tree.** They are kept because the
> behaviour they describe is measured and the reasoning is load-bearing — the
> drop-rate ceiling, the kill-attribution rule and the exclusion strip are all cited
> elsewhere in this project. `check_readme.py` enforces that this list and the
> backticked paths in this file agree, so the two cannot drift apart again.
