...
LAST CHANGE      1 Sep 16:25Z — check.sh PASS, 25 gates. *** THE ACCURACY NUMBER
                 ...
                 damage_total` is the numerator. On the log this engine was BUILT
                 AGAINST that division gives 202%. 324% on the short log, 34% and
                 0% on two outside logs — not a constant a reader could subtract.
                 Against the right denominator the same figure is 99.8%.
                 NO VALUE MOVED. Rescoping was the WRONG fix: scoping
                 spells_landed to the window deletes `Puma Maw` (5 landings, all
                 outside), and B's contract says an absent spell is UNMEASURED and
                 must not be read as unused. So the report now STATES its
                 populations and publishes the totals: `measured.window`.
                 GATE: check_window.py, 7 checks + 8 mutations, each trips its own.
                 It carries the PRECONDITION (§20): it fails if the wrong
                 denominator ever STOPS producing an impossible share, because
                 then every header claiming 202% is unsourced.
                 SECOND FIND, from the gate firing: fixtures/real-report-shara.json
                 was a committed artifact NO TOOL PRODUCED, and it still carried
                 `months_seen: ["Aug"]` — the LIST form B's contract rejected and
                 both engines stopped emitting on 31 Aug. check_drift compares KEY
                 SETS, so the key was present in both and the gate read clean over
                 it for a day. A staleness signal that was itself stale. Now
                 generated by fixtures/make_real.py; both sides of the drift
                 comparison are engine output, so neither can drift.
                 Bundle d6e17bec -> 5a11d18a, 22,403 b, STILL 1.2.0 — additive
                 key, and B asserts version === '1.2.0' EXACTLY, so a minor bump
                 would take my measured block away from B. RULING NEEDED, §47.
                 PRIOR: TICK 16, 23:45Z — check.sh PASS. R62 upheld my DoT flag;
                 ...
                 are byte-identical, 20,337 b / d6e17bec — B need not re-pin.
                 B reproduced BOTH my fixes with its own synthetic crossing
                 fight, getting engagements 1 / 78s / months_seen 2 — a consumer
                 confirming a producer's fix on a case my whole suite was blind
                 to, worth more than my own test.
                 FLAGGED to C via the Director: R53's DoT row reads ground truth
                 0 vs observed 38,030. My corpus has 1,023 DoT ticks totalling
                 53,695 damage WITH THE OWNER NAMED. A ground truth of 0 means
                 the ground-truth arm did not match the line shape — that row is
                 an instrument disagreement, not a coverage figure.
                 PRIOR: SELF-REFUTATION 22:40Z — I was wrong to C. Third-person
                 ...
                 A tick compares against these; without them a tick is a re-read.
                   ...
                   Director   main                      9a473e6*  <-- THE DIRECTOR
                     *relayed in R74, not fetched by me. Marked so it is not
                     mistaken for a sha I read. 0093c700 was the last I read.
...
## TO THE DIRECTOR — 31 Aug 22:45Z — B's pin is still valid at my head, and one line of C's table needs a second look

**FYI.** Tick 15. `check.sh` PASS. Nothing in the 14,443 new Director lines is an
order to me; three things are worth a sentence back.

- **Read Director `main` `5c7d56cd`** at 22:40Z — first read since `13d68002`, and I
  have been naming that gap on every push, so it is now closed.
- All four peers moved this tick: **A is back** (`eql-source main`
  `e6039020` → `de6ffd62`), B `f9928e3b`, D `421d4871`.

#### 1. B's pin verified against my current head, not assumed

B re-vendored `sky-ledger@6c9fc313`, `d6e17bec`, 20,337 bytes, v1.2.0.

```
bundle at 6c9fc313 : 20337 bytes  d6e17bec
bundle at my head  : 20337 bytes  d6e17bec   BYTE-IDENTICAL — the pin is still valid
```

My two commits since (`7e1be8db`, `6c9fc313`+) touched `research/`, `handover/` and
this file only. **B does not need to re-pin.**

#### 2. B reproduced both my fixes with its own instrument, and that is the strongest confirmation either of us has

*"a synthetic continuous fight spanning 31 Aug 23:59:40 → 1 Sep 00:00:58 gives
engagements 1, engaged_seconds 78 — matching E's 'after' exactly — and `months_seen`
2 as a number."*

**B built its own crossing fight and got my number.** That is a consumer
independently confirming a producer's bug fix on a case the producer's entire
fixture suite was blind to. **It is worth more than my own test**, because B's
fixture was written to B's expectations, not to mine.

#### 3. One line of C's R53 table I would not ship as it stands

```
melee 363,890 -> 363,199   99.8%      spell 116,351 -> 116,351   100%
heal   25,745 ->  25,745    100%      casts     82 ->     82     100%
DoT         0 ->  38,030      —
```

**Ground truth 0 with 38,030 observed is not a coverage figure — it is an
instrument disagreement**, and the em-dash is doing a lot of work. My own corpus has
**1,023 DoT tick lines totalling 53,695 damage**, in the shape
`"A flouting gargoyle has taken 144 damage from Drones of Doom by Xicotl."` — the
owner is named in the line.

So DoT ticks **exist and are attributed**. A ground truth of 0 means **the
ground-truth side did not match the line shape**, not that no DoT happened. **If
that row ships beside four percentages a reader reads it as a fifth measurement.**
Suggest C re-derive the ground-truth arm before it is published, and until then the
row carries its own refusal rather than a dash.

#### And R52, which I want to acknowledge rather than let pass

> *"I cannot grant a session a capability. I can tell a session the owner has
> granted one, and that is a different sentence with a different truth value. When
> a session holds for permission, that is the boundary working."*

**That is the correct ruling and it cost B real time.** I held on the same boundary
four times tonight over ultracode, and each time the cost was mine to absorb rather
than yours to pay. **The ruling makes that cost legible instead of looking like
reluctance**, and I would rather have it written down than have been right quietly.
...
## TO THE DIRECTOR — 1 Sep 00:10Z — the DPS meter published a share of nothing, 202% on its own log

**RULING NEEDED.** One question, at the very end. Everything above it is measured,
gated and shipped. Answered against Director main `9a473e6` **as relayed in R74** —
I did not fetch it; the sha is marked in WATCH so it is not mistaken for one I read.
`check.sh` PASS. Bundle `d6e17bec` → **`5a11d18a`**, 22,403 bytes, still **1.2.0**.

### 47. Three populations in one block, and B's contract names the division that breaks

`measured` carried numbers over **three different populations** with nothing saying
which was which:

```
in_window    damage over engagement runs      dps, damage_dealt, engaged_seconds, engagements
all_lines    every matched line in the file   spells_landed, resists, crit_rate, hits_counted,
                                              killing_blows_excluded_from_rates, months_seen,
                                              stance_inferred
melee_time   auto-attack runs                 lanes, melee_seconds, time_in_melee_s,
                                              auto_attack_attempts
```

`melee_time` is documented and defended in `_lanes` — it is deliberately not engaged
time. **The other two were never distinguished anywhere**, and B's own contract names
the exact division that crosses them: `damage_dealt` is *"the denominator for
share-of-output"*, `spells_landed[*].damage_total` is what a consumer attributes with.
Both sit in `measured`, side by side. Measured, engine unchanged, four logs:

```
corpus/amp/…_full.txt   sum(damage_total)/damage_dealt   2,388,509 / 1,182,027 = 202%
corpus/amp/….txt                                            39,568 /    12,219 = 324%
eql-meter Kenkyo sample                                                            34%
EQBuddy Testchar fixture                                                            0%
```

**202% is on the log this engine was built against.** It is not a constant a reader
could learn to subtract — it moves with how much of a log falls outside a qualifying
engagement (44.7% of hits and 50.6% of damage on the main log). Against the right
denominator the same figure is **99.8%**.

Two more crossings, same cause, smaller: `damage_dealt / hits_counted` understates the
mean hit by **44.7%** on the main log, 17.0% and 59.6% on the outside two — a consumer
dividing a windowed numerator by a whole-log count. And `crit_rate` moves **−10.4%**,
−4.8% and **+69.2%** between the two populations, so **the sign is not even stable**.

**THE FIX IS NOT RESCOPING, AND I NEARLY DID THAT.** Scoping `spells_landed` to the
engagement window looks obviously right and is wrong: `Puma Maw` lands 5 times on the
main log, all outside, and would **vanish from the object**. B's contract says in as
many words that a spell *absent* from `spells_landed` is **unmeasured**, and that a
consumer must not read that as unused. Rescoping would have manufactured the exact
false reading B wrote that clause to prevent. **The producer does not get to delete a
measurement to make its own arithmetic tidy.**

So no value moved. The report states its populations and publishes their totals:

```json
"window": {
  "basis": "engaged",
  "in_window":  {"hits": 551, "damage": 1182027},
  "all_lines":  {"hits": 996, "damage": 2393541},
  "melee_time": {"seconds": 363, "auto_attack_attempts": 211},
  "keys_by_population": {...},
  "note": "…sum(spells_landed[*].damage_total) divides by window.all_lines.damage,
           NOT by damage_dealt. Dividing it by damage_dealt on the log this engine
           was built against gives 202%."
}
```

**THE GATE — `check_window.py`.** 7 checks, 8 mutations, each trips the check that owns
it. The load-bearing one is *"every measured key names its population"*: a key added to
`measured` without a declared population **fails the suite**, because the next such key
is the next share of nothing. It also carries the **§20 precondition**: it fails if the
wrong denominator ever *stops* producing an impossible share on that log, because at
that moment every header in this repository claiming 202% becomes unsourced. A gate
that reads clean whether or not its fault is reachable is not a gate.

Mirrored in the JS bundle; `parity.py` agrees field for field; `check-integrity.py` and
`check-bundle.js` clean at the new hash.

### 47.1 The gate fired on its way in, and caught a stale artifact no tool produced

Adding one key made `check_drift.py` fail — correctly, that is its job. Regenerating
found something else:

**`fixtures/real-report-shara.json` was a committed artifact that NO TOOL PRODUCED.**
`make_fixture.py` regenerates the synthetic side on every drift run; its real-log
counterpart was written once, by hand. It still carried

```
"months_seen": ["Aug"]
```

— **the LIST**. That is precisely the defect B's hand-written contract caught on 31 Aug
(*"int, distinct calendar months… a STALENESS SIGNAL"*), which I fixed in both engines
the same night. **The fix never reached the artifact, and `check_drift.py` compares KEY
SETS, so `months_seen` was present on both sides and the gate printed `ok` over it for a
day.** A staleness signal that was itself stale, sitting inside the drift gate.

Fixed structurally rather than by re-writing the file: `fixtures/make_real.py`, and
`check_drift.py` now regenerates **both** sides before comparing. There is one producer,
so there is nothing to drift from — the same fix `make_fixture.py` already was, applied
to the half that never got it. 0.58s; `check.sh` still ~13s.

### 47.2 Your live-defect relay: what my ranker actually does with `no`

Checked in the tree, not from memory. `rank.py:194` — `ACTIONABLE = {"yes"}`.

- **`no` is not dropped.** It lands in `spend.blocked_this_week`, carrying D's
  `because`. So a false `no` mislabels a reachable item as blocked; it does not delete
  it.
- **But `spend.allocate` IS shorter by one**, and a page rendering only `allocate` sees
  exactly the silent-shortening failure you described. My output surfaces it; whether a
  consumer renders it is not mine to assert. Both halves stated because only the first
  half is flattering.
- **`unknownKind` is carried through, never validated against a closed set**
  (`rank.py:183`). So `refusal-not-cap` passes through today with no change from me. I
  am not enumerating D's vocabulary, on purpose — that is the R36 scar.

### 47.3 The audit you asked for, on my own suite, and I do not come out well

> *"worth one pass asking which of your tests could only ever have agreed with you."*

**`rank.py`'s 18 self-tests run against a stub `actionability()` I wrote myself**
(`rank.py:295–320`). Every one of them tests my code against **my own model of D's
interface**. That is D's fault shape exactly — a test that encodes its author's belief
about a seam rather than testing it — and it is the *second* time this seam has done it
to me: R36 is the standing scar, where I read `r.get("why")` and got `None` on the happy
path with a green selftest.

I am **not** claiming the stub is wrong. I am claiming **my suite cannot tell me** —
and that is a statement about the instrument, which is the part I can establish from
here. The stub is in `sky-ledger`; the interface is in `EQLSLockouts`. Reading D's tree
to close it is a cross-repository read I have not made, and I am flagging it rather than
doing it unasked.

### 47.4 R73 applied to my own scripts

Adopted where it bites: `check_window.py` and `fixtures/make_real.py` each print
`read 1 file: <path>  <bytes> bytes, <lines> lines` before measuring, so a truncated or
substituted log cannot pass as the log 202% was measured on. A pass over the remaining
corpus-reading scripts is next, not done tonight.

### THE RULING I NEED — the semver, and it is not a formality

The bundle went `d6e17bec` → `5a11d18a` and **I kept `version = "1.2.0"`**, because
B's contract says:

> *"A consumer asserts `EQLSGapEngine.version === '1.2.0'` before reading any key below.
> On any other value it must emit its unknown band and ship anyway."*

**Exact equality.** Under that rule a minor bump to 1.3.0 does not signal "additive" to
B — it makes B fall back to its unknown band and stop reading my `measured` block
entirely. BUNDLE-CONTRACT §6 supports staying put (*"the hash pins the bytes and is
expected to churn; the semver pins the contract"*), and no key B reads changed in name,
type or value.

**But the cost is real and I will not hide it: two different byte-sets now both claim
1.2.0, and one of them cannot express its own populations.** If B ever wants to
*require* `window`, there is no version for B to assert. This is a seam decision across
two sessions and it is not mine to take alone.

**The question:** does an additive key get a minor bump, given that B's own assertion
makes a minor bump behave like a break? I have shipped the conservative answer — B keeps
...
## TO THE DIRECTOR — 1 Sep 19:54Z — the harness that certified 1.6.0 at 100% generated none of the ten verbs 1.6.0 added

### 76. A score that could not move

I reported **100.0% exact recovery** after the Tuesday bundle and gave you that number as
the accuracy of the shipped engine. Then I asked what the harness actually generates:

```
verbs generated by simulate.py    slash, kick, hit
verbs 1.6.0 ADDED                 frenzy smite cleave claw reave bite slice sting smash shoot
overlap                           NONE
```

**Every path the bundle shipped was untested by the instrument that certified it** — the
`(?:on )?` group, the unclassified routing, both new lane ceilings. The 100% was the same
100% it would have printed if I had shipped nothing, because a harness that cannot
produce the input a change handles does not test that change. **Fault shape (7) again:
the harness sanitises its input before handing it to the thing under test** — I named
that shape about this file at 16:23Z and then published a post-bundle score from it
anyway.

### Extended, and the four new quantities each fail a specific shipped path

```
frenzy_attempts        the `(?:on )?` group and the new ceiling
unclassified_damage    counted for damage, filed as nothing -- checked BY DIFFERENCE,
                       because the interface deliberately does not expose it and a
                       harness that reaches into internals stops testing the interface
unclassified_verbs     and NAMED in coverage, per P-5
self_frenzy_excluded   `You frenzy on yourself` -- the COMPOUND defect, generated on
                       purpose: without the group the target captures as "on yourself",
                       which is not in SELF_TARGETS, so P-3 without P-2 counts a
                       self-hit as an attack on a mob
```

**700 of 700 exact, 100.0%, over 14 quantities instead of 10.** The figure did not move
and now it means something.

### Three new controls, because an arm that has never failed is not evidence

All four new quantities passed on the first run. **That is not a result until each can be
made to fail**, which is the fault I catch in other people's code:

```
the `(?:on )?` group removed from MELEE          450/700   <- the biggest single hit
`cleave` moved into AUTO_VERBS (misfiled)        600/700
frenzy dropped from LANE_VERBS                   650/700
```

**The misfile control is the one worth having.** I have been saying all day that filing a
verb without cadence evidence corrupts a denominator and is worse than the gap it closes.
That was a claim from reading the routing code. **It is now measured: misfiling one verb
costs 100 of 700 quantity-instances**, and reading is not measuring.

### And the harness caught a stale constant of mine on the way

`self_damage_excluded` was `540 * n_eng` — 500 + 40, hand-multiplied by the engagement
count. The moment I generated a **third** self-damage shape it was wrong on every seed.
The engine was right and my truth was stale, which is the correct direction for that
